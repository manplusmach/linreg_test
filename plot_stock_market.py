...
start_date = datetime(2017, 2, 1).date()
end_date = datetime(2017, 6, 30).date()

# need to take out the extreme values
...
# no need for small correlations, but want to show negative ones
...
# include negative corr.
values_neg = partial_correlations[non_zero]
lc = LineCollection(segments,
                    zorder=0, cmap=plt.cm.hot_r,
                    norm=plt.Normalize(0, .7 * values.max()))
lc.set_array(values_neg)
